#!/usr/bin/env python3
"""hello_world_sentinel — the smallest honest sentinel.
...
__manifest__ = {
    ...
    "category": "example",
    "checks": {
        "hello_world": {"domain": "example", "kind": "consistency"},
    },
...
def run(config=None, ctx=None):
    ...
    if not isinstance(cfg.get("greeting"), str) or not cfg["greeting"]:
        return [c["fail"]("hello_world", "greeting must be a non-empty string")]
    return [c["ok"]("hello_world", f"{cfg['greeting']} - the contract holds")]
...
def prove():
    ...
    broken = run({"greeting": ""})
    assert control[0]["ok"] and control[0]["id"] == "hello_world", control
    assert not broken[0]["ok"] and broken[0]["id"] == "hello_world", broken
    return True
